...
def ques_list(request):
    ...
    return render(request , 'ques_list.html' , context = context)
...
def ques_detail(request,id):
    ...
    if request.method == 'POST':
        ...
        if ques.restrict_comments:
            ...
            return HttpResponseRedirect(reverse('test1:ques_detail',args = (id,)))

        if not request.user.is_authenticated:
            return HttpResponseRedirect(reverse('test1:user_login'))
...
def ques_likes(request):
    ...
    return HttpResponseRedirect(reverse('test1:ques_detail',args=(request.POST.get('q_id'),)))
...
def user_login(request):
    if request.user.is_authenticated:
        return HttpResponseRedirect(reverse('test1:ques_list'))
...
def delete_comment(request,id):
    ...
    if cmnt.user != request.user:
        return Http404()
    cmnt.delete()
    return HttpResponseRedirect(reverse('test:ques_detail', args=(id,)))
